...
// blockMsg packages a Valhalla block message and the peer it came from together
...
// invMsg packages a Valhalla inv message and the peer it came from together
// so the block handler has access to that information.
type invMsg struct {
	inv  *wire.MsgInv
	peer *serverPeer
...
// headersMsg packages a Valhalla headers message and the peer it came from
...
// txMsg packages a Valhalla tx message and the peer it came from together
// so the block handler has access to that information.
type txMsg struct {
	tx   *vhcutil.Tx
	peer *serverPeer
...
func (b *blockManager) checkBlockForHiddenVotes(block *vhcutil.Block) {
	var votesFromBlock []*vhcutil.Tx
	...
	var newVotes []*vhcutil.Tx
	var oldTickets []*vhcutil.Tx
	var oldRevocations []*vhcutil.Tx
	oldVoteMap := make(map[chainhash.Hash]struct{},
		int(b.server.chainParams.TicketsPerBlock))
	if template != nil {
		templateBlock := vhcutil.NewBlock(template.Block)
	...
	updatedTxTreeStake := make([]*vhcutil.Tx, 0,
	...
	var updatedTxTreeRegular []*vhcutil.Tx
...
func headerApprovesParent(header *wire.BlockHeader) bool {
	return vhcutil.IsFlagSet16(header.VoteBits, vhcutil.BlockValid)
}
...
func (b *blockManager) handleNotifyMsg(notification *blockchain.Notification) {
	...
	case blockchain.NTNewTipBlockChecked:
		...
		block, ok := notification.Data.(*vhcutil.Block)
		if !ok {
	...
	case blockchain.NTBlockConnected:
		blockSlice, ok := notification.Data.([]*vhcutil.Block)
		if !ok {
		...
		handleConnectedBlockTxns := func(txns []*vhcutil.Tx) {
			for _, tx := range txns {
	...
	case blockchain.NTBlockDisconnected:
		...
		handleDisconnectedBlockTxns := func(txns []*vhcutil.Tx) {
			for _, tx := range txns {
				_, err := txMemPool.MaybeAcceptTransaction(tx, false, true)
				if err != nil && !isDoubleSpendOrDuplicateError(err) {
					txMemPool.RemoveTransaction(tx, true)
...
// queue.
func (b *blockManager) QueueTx(tx *vhcutil.Tx, sp *serverPeer) {
...
func (b *blockManager) QueueBlock(block *vhcutil.Block, sp *serverPeer) {
...
func (b *blockManager) ProcessBlock(block *vhcutil.Block, flags blockchain.BehaviorFlags) (bool, error) {
...
func (b *blockManager) ProcessTransaction(tx *vhcutil.Tx, allowOrphans bool,
	rateLimit bool, allowHighFees bool) ([]*vhcutil.Tx, error) {
...
// pool.
func (b *blockManager) TicketPoolValue() (vhcutil.Amount, error) {
...
// newBlockManager returns a new Valhalla block manager.
...
func removeRegressionDB(dbPath string) error {
	...
	if err == nil {
		vhcdLog.Infof("Removing regression test database from '%s'", dbPath)
		if fi.IsDir() {
...
func warnMultipleDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		vhcdLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB() (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		vhcdLog.Infof("Creating block database in memory.")
	...
	vhcdLog.Infof("Loading block database from '%s'", dbPath)
	...
	vhcdLog.Info("Block database loaded")
	return db, nil
}
