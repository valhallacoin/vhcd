...
// processes to communicate with and and manage the lifetime of a vhcd child
...
// message, shutdown begins.  This prevents vhcd from continuing to run
...
func serviceControlPipeRx(fd uintptr) {
	...
	for {
		...
		if err != nil {
			vhcdLog.Errorf("Failed to read from pipe: %v", err)
			break
		}
	}
...
// for a child vhcd process to communicate with a parent process without the
...
func serviceControlPipeTx(fd uintptr) {
	...
	vhcdLog.Errorf("Failed to write to pipe: %v", err)
}
