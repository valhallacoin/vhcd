...
// FileContents is a string containing the commented example config for vhcd.
...
; The default is ~/.vhcd/data on POSIX OSes, $LOCALAPPDATA/Vhcd/data on Windows,
; ~/Library/Application Support/Vhcd/data on macOS, and $homed/vhcd/data on
; Plan9.  Environment variables are expanded so they may be used.  NOTE: Windows
; environment variables are typically %VARIABLE%, but they must be accessed with
; $VARIABLE here.
; datadir=~/.vhcd/data                            ; Unix
; datadir=$LOCALAPPDATA/Vhcd/data                 ; Windows
; datadir=~/Library/Application Support/Vhcd/data ; macOS
...
; line.  vhcd will not contact 3rd-party sites to obtain external ip addresses.
...
; with, but the behavior is slightly different.  By default, vhcd will query DNS
...
; Disable DNS seeding for peers.  By default, when vhcd starts, it will use
...
; which is used to control and query information from a running vhcd process.
;
...
; All ipv6 interfaces on default port:
;   rpclisten=::
; All interfaces on port 9209:
;   rpclisten=:9209
; All ipv4 interfaces on port 9209:
;   rpclisten=0.0.0.0:9209
; All ipv6 interfaces on port 9209:
;   rpclisten=[::]:9209
; Only ipv4 localhost on port 9209:
;   rpclisten=127.0.0.1:9209
; Only ipv6 localhost on port 9209:
;   rpclisten=[::1]:9209
...
; log level for individual subsystems.  Use vhcd --debuglevel=show to list
