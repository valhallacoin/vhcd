...
type fakeChain struct {
	...
	blocks         map[chainhash.Hash]*vhcutil.Block
...
func (s *fakeChain) FetchUtxoView(tx *vhcutil.Tx, treeValid bool) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) BlockByHash(hash *chainhash.Hash) (*vhcutil.Block, error) {
	s.RLock()
...
func (s *fakeChain) AddBlock(block *vhcutil.Block) {
	s.Lock()
...
func (s *fakeChain) CalcSequenceLock(tx *vhcutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
...
func (s *fakeChain) AddFakeUtxoMedianTime(tx *vhcutil.Tx, txOutIdx uint32, medianTime time.Time) {
	s.Lock()
...
// transactions.
func txOutToSpendableOut(tx *vhcutil.Tx, outputNum uint32, tree int8) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum, Tree: tree},
		amount:   vhcutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
// passed in.
func (p *poolHarness) GetScript(addr vhcutil.Address) ([]byte, error) {
...
func (p *poolHarness) GetKey(addr vhcutil.Address) (chainec.PrivateKey, bool, error) {
	return p.signKey, true, nil
}

// AddFakeUTXO creates a fake mined uxto for the provided transaction.
func (p *poolHarness) AddFakeUTXO(tx *vhcutil.Tx, blockHeight int64) {
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int64, numOutputs uint32) (*vhcutil.Tx, error) {
...
// testing.
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput, numOutputs uint32, mungers ...func(*wire.MsgTx)) (*vhcutil.Tx, error) {
	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput vhcutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*vhcutil.Tx, error) {
	txChain := make([]*vhcutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, vhcutil.NewTx(tx))
...
// output.
func (p *poolHarness) CreateTx(out spendableOutput) (*vhcutil.Tx, error) {
...
func (p *poolHarness) CreateTicketPurchase(sourceTx *vhcutil.Tx, cost int64) (*vhcutil.Tx, error) {
	ticketfee := vhcutil.Amount(singleInputTicketSize)
	ticketPrice := vhcutil.Amount(cost)
	...
	change := vhcutil.Amount(sourceTx.MsgTx().TxOut[0].Value) -
...
func (p *poolHarness) CreateVote(ticket *vhcutil.Tx) (*vhcutil.Tx, error) {
	...
	signedScript, err := txscript.SignTxOutput(p.chainParams, vote, inputToSign,
		...
		p, vote.TxIn[inputToSign].SignatureScript, vhcec.STEcdsaSecp256k1)
	if err != nil {
		return nil, err
	}
	...
	return vhcutil.NewTx(vote), nil
}

// CreateRevocation creates a revocation using the provided ticket.
func (p *poolHarness) CreateRevocation(ticket *vhcutil.Tx) (*vhcutil.Tx, error) {
	...
	signedScript, err := txscript.SignTxOutput(p.chainParams, revocation, inputToSign,
		redeemTicketScript, txscript.SigHashAll, p,
		p, revocation.TxIn[inputToSign].SignatureScript, vhcec.STEcdsaSecp256k1)
	if err != nil {
		return nil, err
	}
	...
	return vhcutil.NewTx(revocation), nil
}
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	payPubKeyAddr, err := vhcutil.NewAddressSecpPubKey(pubKeyBytes,
		chainParams)
	if err != nil {
		return nil, nil, err
	}
	...
	chain := &fakeChain{
		...
		blocks:      make(map[chainhash.Hash]*vhcutil.Block),
...
func testPoolMembership(tc *testContext, tx *vhcutil.Tx, inOrphanPool, inTxPool bool) {
	txHash := tx.Hash()
...
func TestExpirationPruning(t *testing.T) {
	...
	expiringTxns := make([]*vhcutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   vhcutil.Amount(5000000000),
...
func TestSequenceLockAcceptance(t *testing.T) {
	...
	for _, acceptSeqLocks := range []bool{false, true} {
		...
		for i, test := range tests {
			...
			inputTx := vhcutil.NewTx(inputMsgTx)
