...
const (
	defaultConfigFilename        = "vhcd.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "vhcd.log"
	defaultMaxSameIP             = 5
	defaultMaxPeers              = 125
...
var (
	defaultHomeDir     = vhcutil.AppDataDir("vhcd", false)
...
// config defines the configuration options for vhcd.
//
...
type config struct {
	...
	RPCListeners         []string      `long:"rpclisten" description:"Add an interface/port to listen for RPC connections (default port: 9209, testnet: 19209)"`
	...
	MinRelayTxFee        float64       `long:"minrelaytxfee" description:"The minimum transaction fee in VHC/kB to be considered a non-zero fee."`
	...
	AltDNSNames          []string      `long:"altdnsnames" description:"Specify additional dns names to use when generating the rpc server certificate" env:"VHCD_ALT_DNSNAMES" env-delim:","`
	onionlookup          func(string) ([]net.IP, error)
	lookup               func(string) ([]net.IP, error)
	oniondial            func(string, string) (net.Conn, error)
	dial                 func(string, string) (net.Conn, error)
	miningAddrs          []vhcutil.Address
	minRelayTxFee        vhcutil.Amount
...
// createDefaultConfig copies the file sample-vhcd.conf to the given destination path,
...
//
// The above results in vhcd functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	// Update the home directory for vhcd if specified. Since the home
	...
	cfg.minRelayTxFee, err = vhcutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]vhcutil.Address, 0, len(cfg.GetWorkKeys)+
		len(cfg.MiningAddrs))
	for _, strAddr := range cfg.GetWorkKeys {
		addr, err := vhcutil.DecodeAddress(strAddr)
		if err != nil {
	...
	if configFileError != nil {
		vhcdLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// vhcdDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func vhcdDial(network, addr string) (net.Conn, error) {
...
// vhcdLookup returns the correct DNS lookup function to use depending on the
...
func vhcdLookup(host string) ([]net.IP, error) {
