...
func main() {
	...
	method := args[0]
	usageFlags, err := vhcjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := vhcjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// vhcjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(vhcjson.Error); ok {
		...
		// The error is not a vhcjson.Error and this really should not
	...
	marshalledJSON, err := vhcjson.MarshalCmd("1.0", 1, cmd)
	if err != nil {
