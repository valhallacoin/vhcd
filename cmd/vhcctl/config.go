...
const (
	...
	unusableFlags = vhcjson.UFWebsocketOnly | vhcjson.UFNotification
)

var (
	vhcdHomeDir            = vhcutil.AppDataDir("vhcd", false)
	vhcctlHomeDir          = vhcutil.AppDataDir("vhcctl", false)
	vhcwalletHomeDir       = vhcutil.AppDataDir("vhcwallet", false)
	defaultConfigFile      = filepath.Join(vhcctlHomeDir, "vhcctl.conf")
	defaultRPCServer       = "localhost"
	defaultWalletRPCServer = "localhost"
	defaultRPCCertFile     = filepath.Join(vhcdHomeDir, "rpc.cert")
	defaultWalletCertFile  = filepath.Join(vhcwalletHomeDir, "rpc.cert")
)
...
func listCommands() {
	...
	cmdMethods := vhcjson.RegisteredCmdMethods()
	categorized := make([][]string, numCategories)
	for _, method := range cmdMethods {
		flags, err := vhcjson.MethodUsageFlags(method)
		if err != nil {
		...
		usage, err := vhcjson.MethodUsageText(method)
		if err != nil {
			// This should never happen since the method was just
			// returned from the package, but be safe.
			continue
		...
		if flags&vhcjson.UFWalletOnly != 0 {
...
// config defines the configuration options for vhcctl.
//
...
// For this it tries to read the vhcd config file at its default path, and extract
// the RPC user and password from it.
func createDefaultConfigFile(destinationPath string) error {
	// Nothing to do when there is no existing vhcd conf file at the default
	// path to extract the details from.
	vhcdConfigPath := filepath.Join(vhcdHomeDir, "vhcd.conf")
	if !fileExists(vhcdConfigPath) {
		return nil
	}

	// Read vhcd.conf from its default path
	vhcdConfigFile, err := os.Open(vhcdConfigPath)
	if err != nil {
		return err
	}
	defer vhcdConfigFile.Close()
	content, err := ioutil.ReadAll(vhcdConfigFile)
	if err != nil {
		return err
	}
