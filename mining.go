...
func containsTxIns(txs []*vhcutil.Tx, tx *vhcutil.Tx) bool {
...
// can not be found.
func txIndexFromTxList(hash chainhash.Hash, list []*vhcutil.Tx) int {
	for i, tx := range list {
		h := tx.Hash()
		if hash == *h {
			return i
...
func UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int64, extraNonce uint64) error {
	...
	// TODO(davec): A vhcutil.Block should use saved in the state to avoid
	// recalculating all of the other transaction hashes.
	// block.Transactions[0].InvalidateCache()

	// Recalculate the merkle root with the updated extra nonce.
	block := vhcutil.NewBlockDeepCopyCoinbase(msgBlock)
...
func createCoinbaseTx(subsidyCache *blockchain.SubsidyCache, coinbaseScript []byte, opReturnPkScript []byte, nextBlockHeight int64, addr vhcutil.Address, voters uint16, params *chaincfg.Params) (*vhcutil.Tx, error) {
	tx := wire.NewMsgTx()
	...
	if nextBlockHeight == 1 && len(params.BlockOneLedger) != 0 {
		...
		addrs := make([]vhcutil.Address, len(params.BlockOneLedger))
		for i, payout := range params.BlockOneLedger {
			addr, err := vhcutil.DecodeAddress(payout.Address)
			if err != nil {
				return nil, err
			}
			addrs[i] = addr
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *vhcutil.Tx, height int64) error {
	for _, txIn := range tx.MsgTx().TxIn {
...
func logSkippedDeps(tx *vhcutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
func maybeInsertStakeTx(bm *blockManager, stx *vhcutil.Tx, treeValid bool) bool {
	missingInput := false
...
func deepCopyBlockTemplate(blockTemplate *BlockTemplate) *BlockTemplate {
	...
	coinbaseCopy :=
		vhcutil.NewTxDeep(blockTemplate.Block.Transactions[0])
...
func handleTooFewVoters(subsidyCache *blockchain.SubsidyCache, nextHeight int64, miningAddress vhcutil.Address, bm *blockManager) (*BlockTemplate, error) {
	...
	if nextHeight >= stakeValidationHeight {
		if bm.AggressiveMining {
			if curTemplate != nil {
				...
				block := vhcutil.NewBlockDeepCopyCoinbase(cptCopy.Block)
			...
			if curTemplate == nil {
				...
				btBlockTemp := vhcutil.NewBlockDeepCopyCoinbase(btMsgBlock)
				merkles :=
				...
				// Make sure the block validates.
				btBlock := vhcutil.NewBlockDeepCopyCoinbase(btMsgBlock)
...
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress vhcutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*vhcutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		// Consider caching this in the mempool in the future. - Valhalla
	...
	blockTxnsStake := make([]*vhcutil.Tx, 0, len(blockTxns))
	...
	for _, tx := range blockTxns {
		...
		if stake.IsSSGen(msgTx) {
			txCopy := vhcutil.NewTxDeepTxIns(msgTx)
	...
	if nextBlockHeight < stakeValidationHeight {
		...
		for _, vb := range voteBitsVoters {
			if vhcutil.IsFlagSet16(vb, vhcutil.BlockValid) {
				voteYea++
			}
			totalVotes++
		}
		...
		if votebits == uint16(0x0000) {
			...
			tempBlockTxns := make([]*vhcutil.Tx, 0, len(sourceTxns))
			for _, tx := range blockTxns {
				if tx.Tree() == wire.TxTreeRegular {
					...
					if isValid {
						txCopy := vhcutil.NewTxDeepTxIns(tx.MsgTx())
	...
	// Valhalla: We need to move this downwards because of the requirements
	...
	blockTxnsRegular := make([]*vhcutil.Tx, 0, len(blockTxns)+1)
	...
	minrLog.Debugf("Created new block template (%d transactions, %d "+
		...
		vhcutil.Amount(msgBlock.Header.SBits).ToCoin())
