...
// nodeConfig contains all the args, and data required to launch a vhcd process
...
// arguments returns an array of arguments that be used to launch the vhcd
// process.
...
// command returns the exec.Cmd which will be used to start the vhcd process.
...
// to the vhcd process that is launched via Start().
...
// as the base for the log and data directories for vhcd.
...
// start creates a new vhcd process, and writes its pid in a file reserved for
...
func (n *node) start() error {
	...
	pid, err := os.Create(filepath.Join(n.config.String(), "vhcd.pid"))
	if err != nil {
		return err
	}
...
// stop interrupts the running vhcd process process, and waits until it exits
...
// shutdown terminates the running vhcd process, and cleans up all
