...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := vhcutil.Hash160(redeemScript)
...
// subsidy.
func additionalCoinbasePoW(amount vhcutil.Amount) func(*wire.MsgBlock) {
...
func additionalCoinbaseDev(amount vhcutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
		// Increase the first proof-of-work coinbase subsidy by the
		// provided amount.
...
func additionalSpendFee(fee vhcutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
func Generate(includeLargeReorg bool) (tests [][]TestInstance, err error) {
	...
	g.NextBlock("bpi1", nil, nil, func(b *wire.MsgBlock) {
		spendOut := chaingen.MakeSpendableOut(g.Tip(), 0, 0)
		ticketPrice := vhcutil.Amount(g.CalcNextRequiredStakeDifficulty())
	...
	g.NextBlock("bbadtaxscript", outs[5], ticketOuts[5],
		func(b *wire.MsgBlock) {
			...
			p2shTaxAddr := addrs[0].(*vhcutil.AddressScriptHash)
			p2pkhTaxAddr, err := vhcutil.NewAddressPubKeyHash(
	...
	g.NextBlock("bv15", outs[9], ticketOuts[9], func(b *wire.MsgBlock) {
		ticketFee := vhcutil.Amount(2)
		ticketPrice := vhcutil.Amount(g.CalcNextReqStakeDifficulty(g.Tip()))
		ticketPrice--
	...
	// vector is not possible in Valhalla since the block size is included in
	...
	// Valhalla enforces the coinbase includes the block height to which it
	...
	g.NextBlock("bcb9", outs[19], ticketOuts[19], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = vhcutil.MaxAmount + 1
	})
	rejected(blockchain.ErrBadTxOutValue)
	...
	g.NextBlock("bcb10", outs[19], ticketOuts[19], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = vhcutil.MaxAmount
