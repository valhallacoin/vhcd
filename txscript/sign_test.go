...
func mkGetKey(keys map[string]addressToKey) KeyDB {
	if keys == nil {
		return KeyClosure(func(addr vhcutil.Address) (chainec.PrivateKey,
			bool, error) {
...
func mkGetScript(scripts map[string][]byte) ScriptDB {
	if scripts == nil {
		return ScriptClosure(func(addr vhcutil.Address) (
			[]byte, error) {
			return nil, errors.New("nope 3")
		})
	}
	return ScriptClosure(func(addr vhcutil.Address) ([]byte,
		error) {
...
func signAndCheck(msg string, tx *wire.MsgTx, idx int, pkScript []byte,
	...
	previousScript []byte, suite vhcec.SignatureType) error {
...
func TestSignTxOutput(t *testing.T) {
	...
	signatureSuites := []vhcec.SignatureType{
		vhcec.STEcdsaSecp256k1,
		vhcec.STEd25519,
		vhcec.STSchnorrSecp256k1,
	}
	tx := &wire.MsgTx{
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				address, err := vhcutil.NewAddressPubKeyHash(
					vhcutil.Hash160(pkBytes), testingParams,
					suite)
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			if err := signAndCheck(msg, tx, i, pkScript, hashType,
				...
				vhcec.STEcdsaSecp256k1); err != nil {
				t.Error(err)
				break
			}
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				// for more details
				address, err := vhcutil.NewAddressSecpPubKeyCompressed(pk,
					testingParams)
				if err != nil {
					t.Errorf("failed to make address for %s: %v",
						msg, err)
		...
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				case vhcec.STEd25519:
					...
					key, pk = chainec.Edwards.PrivKeyFromBytes(keyDB)
					pkBytes = pk.SerializeUncompressed()
					address, err = vhcutil.NewAddressEdwardsPubKey(pkBytes,
						testingParams)
					if err != nil {
						t.Errorf("failed to make address for %s: %v",
							msg, err)
				...
				case vhcec.STSchnorrSecp256k1:
					...
					key, pk = chainec.SecSchnorr.PrivKeyFromBytes(keyDB)
					pkBytes = pk.Serialize()
					address, err = vhcutil.NewAddressSecSchnorrPubKey(pkBytes,
						testingParams)
					if err != nil {
						t.Errorf("failed to make address for %s: %v",
							msg, err)
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				address, err := vhcutil.NewAddressPubKeyHash(
					vhcutil.Hash160(pkBytes), testingParams, suite)
				if err != nil {
					t.Errorf("failed to make address for %s: %v",
						msg, err)
					break
				...
				scriptAddr, err := vhcutil.NewAddressScriptHash(
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			address1, err := vhcutil.NewAddressSecpPubKeyCompressed(pk1,
				testingParams)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
			...
			address2, err := vhcutil.NewAddressSecpPubKeyCompressed(pk2,
				testingParams)
			if err != nil {
			...
			pkScript, err := MultiSigScript(
				[]*vhcutil.AddressSecpPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
			...
			if err := signAndCheck(msg, tx, i, scriptPkScript,
				...
				mkGetKey(map[string]addressToKey{
					...
					scriptAddr.EncodeAddress(): pkScript,
				}), nil, vhcec.STEcdsaSecp256k1); err != nil {
				t.Error(err)
				break
			}
...
var (
	...
	thisAddressUnc, _ = vhcutil.NewAddressPubKeyHash(
		vhcutil.Hash160(thisPubKey.SerializeUncompressed()),
		testingParams, vhcec.STEcdsaSecp256k1)
	uncompressedPkScript, _ = PayToAddrScript(thisAddressUnc)
	thisAddressCom, _       = vhcutil.NewAddressPubKeyHash(
		vhcutil.Hash160(thisPubKey.SerializeCompressed()),
		testingParams, vhcec.STEcdsaSecp256k1)
