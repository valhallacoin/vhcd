...
// newAddressPubKey returns a new vhcutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) vhcutil.Address {
	pubkey, err := secp256k1.ParsePubKey(serializedPubKey)
	if err != nil {
		panic("invalid public key in test source")
	}
	addr, err := vhcutil.NewAddressSecpPubKeyCompressed(pubkey,
...
// newAddressPubKeyHash returns a new vhcutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) vhcutil.Address {
	addr, err := vhcutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams,
...
// newAddressScriptHash returns a new vhcutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) vhcutil.Address {
	addr, err := vhcutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the vhcutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the vhcutil.Address interface.
...
// DSA returns -1.
func (b *bogusAddress) DSA(chainParams *chaincfg.Params) vhcec.SignatureType {
	return -1
}
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := vhcutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := vhcutil.NewAddressScriptHashFromHash(hexToBytes("e8c30"+
	...
	p2pkCompressedMain, err := vhcutil.NewAddressSecpPubKey(hexToBytes("02192d7"+
	...
	p2pkCompressed2Main, err := vhcutil.NewAddressSecpPubKey(hexToBytes("03b0b"+
	...
	tests := []struct {
		...
		// pay-to-pubkey address on mainnet. for Valhalla this would
		// be uncompressed, but standard for Valhalla is 33 byte
		...
		{(*vhcutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*vhcutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*vhcutil.AddressSecpPubKey)(nil), "", errUnsupportedAddress},
		{(*vhcutil.AddressEdwardsPubKey)(nil), "", errUnsupportedAddress},
		{(*vhcutil.AddressSecSchnorrPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	//  mainnet p2pk 13CG6SJ3yHUXo4Cr2RY4THLLJrNFuG3gUg
	p2pkCompressedMain, err := vhcutil.NewAddressSecpPubKey(hexToBytes("02192d"+
	...
	tests := []struct {
		keys      []*vhcutil.AddressSecpPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*vhcutil.AddressSecpPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
		...
		{
			// By default compressed pubkeys are used in Valhalla.
			[]*vhcutil.AddressSecpPubKey{
				p2pkUncompressedMain.(*vhcutil.AddressSecpPubKey),
			},
			1,
...
func TestGenerateSStxAddrPush(t *testing.T) {
	...
	for _, test := range tests {
		addr, err := vhcutil.DecodeAddress(test.addrStr)
		if err != nil {
