...
const (
	// svcName is the name of vhcd service.
	svcName = "vhcdsvc"
	...
	svcDisplayName = "Vhcd Service"

	// svcDesc is the description of the service.
	svcDesc = "Downloads and stays synchronized with the Valhalla block " +
...
// logServiceStartOfDay logs information about vhcd when the main server has
...
// vhcdService houses the main service handler which handles all service
// updates and launching vhcdMain.
type vhcdService struct{}
...
// long-running vhcdMain (which is the real meat of vhcd), handles service
// change requests, and notifies the service control manager of changes.
func (s *vhcdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start vhcdMain in a separate goroutine so the service can start
...
// installService attempts to install the vhcd service.  Typically this should
...
// removeService attempts to uninstall the vhcd service.  Typically this should
...
// startService attempts to start the vhcd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &vhcdService{})
	if err != nil {
