...
func (stats *Estimator) EstimateFee(targetConfs int32) (vhcutil.Amount, error) {
	stats.lock.RLock()
	...
	return vhcutil.Amount(rate), nil
}
...
func (stats *Estimator) AddMemPoolTransaction(txHash *chainhash.Hash, fee, size int64, txType stake.TxType) {
	...
	// minimum at values less than 0.001 VHC/KB. This is needed because due to
...
func (stats *Estimator) ProcessBlock(block *vhcutil.Block) error {
	stats.lock.Lock()
	defer stats.lock.Unlock()
