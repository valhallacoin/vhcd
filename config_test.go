...
//   os.Setenv("VHCD_ALT_DNSNAMES", "hostname1,hostname2")
//
...
func TestLoadConfig(t *testing.T) {
	_, _, err := loadConfig()
	if err != nil {
		t.Fatalf("Failed to load vhcd config: %s", err)
	}
}
...
// TestAltDNSNamesWithEnv ensures the VHCD_ALT_DNSNAMES environment variable is
